...
void ImprovWiFi::sendDeviceUrl(ImprovTypes::Command cmd)
{
    ...
    if (customGetLocalIpCallback) {
        ipStr = customGetLocalIpCallback();
    }
...
void ImprovWiFi::setCustomGetLocalIpCallback(CustomGetLocalIpCallback *getLocalIpCallback)
{
    customGetLocalIpCallback = getLocalIpCallback;
}


std::vector<std::string> ImprovWiFi::split(std::string s, std::string delimiter)
{
...
void ImprovWiFi::getWifiNetworks()
{
    unsigned char apBuffer[256] = {0};
