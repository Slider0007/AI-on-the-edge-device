            ...
            if (NUMBERS[j]->isFallbackValueValid) {
                ...
                if (NUMBERS[j]->useMaxRateValue) {
                    ...
                    if (abs(RatePerSelection) > abs((double)NUMBERS[j]->maxRateValue)) {
                        ...
                        LogFile.WriteToFile(ESP_LOG_WARN, TAG, "Sequence: " + NUMBERS[j]->name + ", Status: " + NUMBERS[j]->sValueStatus);           
                        NUMBERS[j]->isActualValueConfirmed = false;
                        FlowStateHandlerSetError(-1, true); // Set warning code for post cycle error handler 'doAutoErrorHandling' (only warning)
                    }
                }
...
void ClassFlowPostProcessing::doAutoErrorHandling()
{
    ...
    if (SaveDebugInfo && getFlowState()->ErrorCode == -1) {  // If saving debug infos enabled and "rate to high" event
        time_t actualtime;
        time(&actualtime);
            ...
            if (NUMBERS[j]->digit_roi)
                for (int i = 0; i < NUMBERS[j]->digit_roi->ROI.size(); ++i)
                    NUMBERS[j]->digit_roi->ROI[i]->image_org->SaveToFile(destination + "/" + NUMBERS[j]->name + "_dig" + std::to_string(i+1) + "_" +
                                to_stringWithPrecision(NUMBERS[j]->digit_roi->ROI[i]->CNNResult/10.0, NUMBERS[j]->decimalPlaceCount) + "_org.jpg");
            ...
            if (NUMBERS[j]->analog_roi)
                for (int i = 0; i < NUMBERS[j]->analog_roi->ROI.size(); ++i)
                    NUMBERS[j]->analog_roi->ROI[i]->image_org->SaveToFile(destination + "/" + NUMBERS[j]->name + "_ana" + std::to_string(i+1) + "_" +
                                to_stringWithPrecision(NUMBERS[j]->analog_roi->ROI[i]->CNNResult/10.0, NUMBERS[j]->decimalPlaceCount) + "_org.jpg");
        }
        
        LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "doAutoErrorHandling: Rate too high, debug infos saved: " + destination);
    }
}
