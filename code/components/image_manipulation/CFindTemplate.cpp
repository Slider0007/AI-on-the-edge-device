...
bool IRAM_ATTR CFindTemplate::findTemplate(AlignmentMarker *_ref, bool _noFast)
{
    ...
    if (_ref->alignment_algo == ALIGNALGO_FAST && _ref->fastalg_x > 0 && _ref->fastalg_y > 0 && !_noFast) {
    ...
    if (_ref->alignment_algo == ALIGNALGO_DEFAULT || _ref->alignment_algo == ALIGNALGO_FAST) {
