...
CImageBasis::CImageBasis(std::string _name, CImageBasis *_copyfrom)
{
    ...
    rgb_image = (unsigned char *)malloc_psram_heap(std::string(TAG) + "->CImageBasis (" + name + ")", memsize,
...
void CImageBasis::resizeImage(int _new_dx, int _new_dy)
{
    ...
    rgb_image = (unsigned char *)malloc_psram_heap(std::string(TAG) + "->CImageBasis resizeImage (" + name + ")", memsize,
                                                   MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
...
ImageData *CImageBasis::writeToMemoryAsJPG(const int quality)
{
    ...
    if (jpgFileTooLarge) {
        ...
        LogFile.writeToFile(ESP_LOG_ERROR, TAG,
                            "writeToMemoryAsJPG: Creation aborted! JPG size > preallocated buffer: " + std::to_string(MAX_JPG_SIZE));
    }
    return ii;
}
