...
void test_doFlowPP()
{
    ...
    analogs = {6.4};
    expected = "376529.6";
    result = processDoFlow(digits, analogs);
    TEST_ASSERT_EQUAL_STRING(expected, result.c_str());
...
void test_doFlowPP1()
{
    ...
    // https://github.com/jomjol/AI-on-the-edge-device/issues/942#issuecomment-1226966346
    digits = {9.9, 2.8, 2.9, 2.9, 3.7, 0.1};
    analogs = {};
    expected = "33340";
    result = processDoFlow(digits, analogs);
    TEST_ASSERT_EQUAL_STRING(expected, result.c_str());
    ...
    digits = {9.9, 6.8, 1.1, 4.7, 2.7, 6.0, 9.0, 2.8}; // 4.7 needs to be 5
    analogs = {};
    expected = "7153692";
    result = processDoFlow(digits, analogs);
    TEST_ASSERT_EQUAL_STRING(expected, result.c_str());
...
void test_doFlowPP2()
{
    ...
    std::string result = processDoFlow(digits, analogs, CNNTYPE_DIGIT_CLASS11);
    TEST_ASSERT_EQUAL_STRING(expected, result.c_str());
    ...
    result = processDoFlow(digits, analogs, CNNTYPE_DIGIT_CLASS100, false, -3);
    TEST_ASSERT_EQUAL_STRING(expected, result.c_str());

    // extendResolution=true
    result = processDoFlow(digits, analogs, CNNTYPE_DIGIT_CLASS100, true, -3);
    TEST_ASSERT_EQUAL_STRING(expected_extended, result.c_str());

    // checkConsistency=true
    result = processDoFlow(digits, analogs, CNNTYPE_DIGIT_CLASS11, false, -3, true);
    TEST_ASSERT_EQUAL_STRING(expected, result.c_str());
    ...
    result = processDoFlow(digits, analogs, CNNTYPE_DIGIT_CLASS100);
    TEST_ASSERT_EQUAL_STRING(expected, result.c_str());

    // checkConsistency=true --> checkConsistency only works with CLASS11 models (1-9 + NaN)
    // result = processDoFlow(digits, analogs, CNNTYPE_DIGIT_CLASS11, false, 0, true);
    // TEST_ASSERT_EQUAL_STRING(expected, result.c_str());

    // extendResolution=true
    result = processDoFlow(digits, analogs, CNNTYPE_DIGIT_CLASS100, true);
    TEST_ASSERT_EQUAL_STRING(expected_extended, result.c_str());

    // checkConsistency=true und extendResolution=true --> checkConsistency only works with CLASS11 models (1-9 + NaN)
    // result = processDoFlow(digits, analogs, CNNTYPE_DIGIT_CLASS11, true, 0, true);
    // TEST_ASSERT_EQUAL_STRING(expected_extended, result.c_str());
...
void test_doFlowPP3()
{
    ...
    // extendResolution=false
    std::string result = processDoFlow(digits, analogs, CNNTYPE_DIGIT_CLASS100);
    TEST_ASSERT_EQUAL_STRING(expected, result.c_str());
    ...
    // letztes digit läuft mit analog zeiger mit. Hier nur lösbar mit setAnalogDigitSyncValue=7.7
    analogs = {8.7};
    expected = "170.0528";
    expected_extended = "170.05287";

    // extendResolution=false
    UnderTestPost *undertestPost = initDoFlow(digits, analogs, CNNTYPE_DIGIT_CLASS100, false, -3);
    setAnalogDigitSyncValue(7.7);
    result = processDoFlow(undertestPost);
    TEST_ASSERT_EQUAL_STRING(expected, result.c_str());
    delete undertestPost;

    // checkConsistency=false und extendResolution=true
    undertestPost = initDoFlow(digits, analogs, CNNTYPE_DIGIT_CLASS100, true, -3);
    setAnalogDigitSyncValue(7.7);
    result = processDoFlow(undertestPost);
    TEST_ASSERT_EQUAL_STRING(expected_extended, result.c_str());
    delete undertestPost;
    ...
    // extendResolution=false
    undertestPost = initDoFlow(digits, analogs, CNNTYPE_DIGIT_CLASS100, false, -3);
    setAnalogDigitSyncValue(7.7);
    result = processDoFlow(undertestPost);
    TEST_ASSERT_EQUAL_STRING(expected, result.c_str());
    delete undertestPost;
