...
void testNegative()
{
    ...
    UnderTestPost *underTestPost = initDoFlow(digits, analogs);
    ...
    underTestPost = initDoFlow(digits, analogs, true);
    setAllowNegative(false);
    setFallbackValue(underTestPost, fallbackValue_extended);
    result = processDoFlow(underTestPost);
    TEST_ASSERT_EQUAL_MEMORY("E91 Rate negative", underTestPost->sequenceDataPtr->sValueStatus.c_str(), 17);
    ...
    underTestPost = initDoFlow(digits, analogs);
    setAllowNegative(false);
    setFallbackValue(underTestPost, fallbackValue_extended);
    result = processDoFlow(underTestPost);
    TEST_ASSERT_EQUAL_MEMORY("E91 Rate negative", underTestPost->sequenceDataPtr->sValueStatus.c_str(), 17);
...
void testNegative_Issues()
{
    ...
    UnderTestPost *underTestPost = initDoFlow(digits, analogs, false, -2);
    setAllowNegative(false);
    setFallbackValue(underTestPost, fallbackValue);
    std::string result = processDoFlow(underTestPost);
    TEST_ASSERT_EQUAL_MEMORY("E91 Rate negative", underTestPost->sequenceDataPtr->sValueStatus.c_str(), 17);
